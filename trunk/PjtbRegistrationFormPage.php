...
<div class="row">
<div class="label">Email Address<span class="small">Password resets will be sent to this address</span></div>
<input type="text" id="emailfield" name="email" maxlength="254" />
</div>
...
function showEmailHint(e) {
	if (!hintVisible($(this)) && $(this).val().length == 0) {
		var message = "Please provide a valid email. This will become the sole means of our administrators contacting you for account related settings";
		showHint(message, $(this), "250px", false);
		$(this).on('blur', function(e2) {
			if (hintTextMatch($(this), message))
				hideHint($(this));
...
function emailChanged(e) {
	...
	if (!emailValid($(this).val())) {
		emailOk = false;
		showHint("Not a valid email address", $(this), "250px", true);
	} else {
		emailOk = true;
	}
	updateSubmitButton();
...
$(document).ready(function() {
	...
	$('#emailfield').on('click', showEmailHint).on('keyup', emailChanged);
